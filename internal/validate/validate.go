...
// Alias checks that the alias contains only lowercase letters, digits, and hyphens,
...
// Message checks that the message is non-empty and at most 32768 characters.
...
// Headers checks that the headers map has at most 20 entries.
...
// BroadcastAliases checks that the aliases slice has between 1 and 20 items.
// Returns an error if empty or exceeds the maximum.
func BroadcastAliases(aliases []string) error {
...
// PingEndpoint checks that a ping endpoint path starts with "/" and is at most 256 characters.
// Returns an error if invalid.
